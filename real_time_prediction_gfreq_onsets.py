    ...
    def main(self, F):
        ...
        while True:
            ...
            if emg_data.shape[1] >= self.n:
                ...
                if f_0_norm > self.thresh and time.time() - start_time >= 3:
                ...
                prev_O = O
                time.sleep(0.025)  # k value between two adjacent TMA maps
...
def main():
    ...
    # get the graph fourier matrix
    d, V = np.linalg.eig(A)
    D = np.diag(np.sort(d))
    lambda_sorted = np.sort(d)
    V = V[:, np.argsort(d)]
    # V[V < 1e-5] = 0
    F = np.linalg.inv(V)

    # load the models
    model_path = '/Users/ashwin/Current Work/Real-Time Hand Gesture Recognition with TMA Maps/src/gsp/subject_2001/model_gnn_rnn.pt'
    model = Net()
    ...
    el = EmgGraphLearn(fs=200,
                       no_channels=8,
                       obs_dur=0.400)
    listener = EmgCollectorPrediction(n=1000)
